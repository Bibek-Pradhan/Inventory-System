 ...
 * @author nishan
 */
public class It_Store extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        warranty_label1 = new javax.swing.JLabel();
        ...
        warranty_label1.setFont(new java.awt.Font("Verdana", 3, 14)); // NOI18N
        warranty_label1.setForeground(new java.awt.Color(0, 204, 153));
        warranty_label1.setText("Brand:");
        ...
        tblInfo.setFont(new java.awt.Font("Segoe UI", 3, 14)); // NOI18N
        tblInfo.setForeground(new java.awt.Color(254, 254, 254));
        tblInfo.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "ID", "Name", "Category", "Quality", "Model Number", "Price", "Warrenty", "Brand"
            }
        ) {
        ...
        appliance_available_search_button.setText("Search Category");
        ...
        getContentPane().add(app_pannel, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, -1, -1));
    ...
    private void aboutApp() {
        ...
        JOptionPane.showMessageDialog(rootPane, ""
                ...
                "What it's For ?", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void addItems() {
        ...
        if (!id.isEmpty() && !name.isEmpty() && category_combobox.getSelectedIndex() != 0 && !model_number_input.isEmpty() && !price_input.isEmpty() && warranty_combobox.getSelectedIndex() != 0 && !brand.isEmpty() && !quality_input.isEmpty()) {
            for (WarehouseData rd : warehouseList) {
                if (id.equals(rd.getId())) {
                    JOptionPane.showMessageDialog(rootPane, "Id can't be repeat.", "Error", 0);
                    return;
                } else if (model_number_input.equals(rd.getModuleNumber())) {
                    JOptionPane.showMessageDialog(rootPane, "Model number of the product can't be repeated.", "Error", 0);
                    return;
                }
            ...
            String arr[] = {id, name, category, quality_input, model_number_input, price_input, warranty_input, brand};
            model.addRow(arr);

            JOptionPane.showMessageDialog(rootPane, "Product added successfully.", "Message", 1);
            clearScreen();

            int price_item = Integer.parseInt(price_input);
            WarehouseData obj = new WarehouseData(id, name, category, quality_input, model_number_input, price_input, warranty_input, brand);
            warehouseList.add(obj);

            Collections.sort(warehouseList, item_price);

            // for checking sorted price
//            for(WarehouseData str : warehouseList){
//                System.out.println(str.getPrice());
//            }
        } else if (id.isEmpty() || name.isEmpty() || model_number_input.isEmpty() || price_input.isEmpty() || brand.isEmpty()) {
            JOptionPane.showMessageDialog(rootPane, "Please fill all the field and Try Again.", "Empty Field", JOptionPane.ERROR_MESSAGE);
            ...
            JOptionPane.showMessageDialog(rootPane, "Please select any one warranty scheme.", "Empty Warranty", JOptionPane.ERROR_MESSAGE);
        } else {
            JOptionPane.showMessageDialog(rootPane, "Please, fill all the field and Try Again.", "Empty Field", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    // sorting ArrayList
    public static Comparator<WarehouseData> item_price = new Comparator<WarehouseData>() {
        public int compare(WarehouseData d1, WarehouseData d2) {
            String price_get1 = d1.getPrice();
            String price_get2 = d2.getPrice();

            int price1 = Integer.parseInt(price_get1);
            int price2 = Integer.parseInt(price_get2);
    ...
    private void searchByPrice() {
        ...
        if (!price.isEmpty()) {
            int price_search = Integer.parseInt(price);

            int index = Collections.binarySearch(warehouseList, new WarehouseData(null, null, null, null, null, price, null, null), item_price);

            if (index >= 0) {
                for (WarehouseData sh : warehouseList) {
                    JOptionPane.showMessageDialog(rootPane, "Price found. The details of the price are: /n" + sh.getId() + sh.getPrice(), "Message", 1);
                    break;
                }
            } else {
                JOptionPane.showMessageDialog(rootPane, "No price found", "Error", 0);
            }
        } else {
            JOptionPane.showMessageDialog(rootPane, "Please insert price first.", "Error", 0);
        }
    }

    // importing files in table
...
//                    WarehouseData obj = new WarehouseData(row);
//                    warehouseList.add(obj);
//                    
//                    
//                   
...
//                Logger.getLogger(It_Store.class.getName()).log(Level.SEVERE, null, ex);
//            }
//
//        } else {
...
//    }
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            if (tblInfo.getRowCount() != 0) {
                try {
                    ...
                    // for the name of the table in excel
                    ...
                    for (int i = 0; i < tblInfo.getRowCount(); i++) {
                        for (int j = 0; j < tblInfo.getColumnCount(); j++) {
                            //write the value of table 
                            bw.write(tblInfo.getValueAt(i, j).toString() + ",");
                        }
                        bw.newLine();//record per line 
                    }
                    JOptionPane.showMessageDialog(this, "Sucessfully saved.", "Message", JOptionPane.INFORMATION_MESSAGE);
                    bw.close();
                    fw.close();
                ...
                JOptionPane.showMessageDialog(this, "Table is empty. Please, fll the table first.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(It_Store.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
    ...
    private javax.swing.JLabel warranty_label1;
